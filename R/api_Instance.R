api_create_instance <- function(instance_settings, is_default = FALSE) {
  super <- NULL # satisfy linter

  api <- APIInstanceAPI$new(instance_settings = instance_settings)
  ...
  api_check_requires("Connecting to Python", "reticulate", alert = "warning")
  py_lamin <- tryCatch(
...
#'
#' @description
#' Connect to a LaminDB instance using the [api_connect()] function.
#' The instance object provides access to the modules and registries
#' of the instance.
#'
#' @details
#' Note that by connecting to an instance via [api_connect()], you receive
#' a "richer" version of the APIInstance class documented here, providing
...
#' db <- api_connect("laminlabs/cellxgene")
#'
#' # fetch an artifact
...
#' artifact$load()
#' }
APIInstance <- R6::R6Class( # nolint object_name_linter
  "APIInstance",
  cloneable = FALSE,
  public = list(
    #' @description
    ...
    initialize = function(settings, api, schema, is_default, py_lamin) {
      ...
      private$.module_classes <- purrr::map(
        names(schema),
        ...
        purrr::set_names(names(schema))
    },
    #' @description Get the modules for the instance.
    #'
    #' @return A list of [APIModule] objects.
    ...
    #' @description
    #' Create a string representation of an `APIInstance`
    #'
    #' @param style Logical, whether the output is styled using ANSI codes
    #'
