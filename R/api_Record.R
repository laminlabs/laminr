api_create_record_class <- function(instance, registry, api) {
...
#' @noRd
api_create_temporary_record_class <- function(record_class) { # nolint object_length_linter
...
APIRecord <- R6::R6Class( # nolint object_name_linter
  "APIRecord",
  cloneable = FALSE,
  public = list(
    #' @description
    ...
    initialize = function(instance, registry, api, data) {
      ...
      expected_fields <-
        registry$get_fields() |>
        purrr::discard(~ is.null(.x$column_name)) |>
        purrr::map_chr("column_name") |>
        unname()
    ...
    #' @description
    #' Create a string representation of a `APIRecord`
    #'
    #' @param style Logical, whether the output is styled using ANSI codes
    #'
    ...
    to_string = function(style = FALSE) {
      ...
      record_fields <- purrr::map(names(expected_fields), function(.field) {
        value <- tryCatch(
          self[[.field]],
          error = function(err) {
        ...
        if (inherits(value, "APIRecord")) {
          value <- value$id
        }

        value
      }) |>
        purrr::set_names(expected_fields)
  ...
  private = list(
    ...
    get_value = function(key) {
      ...
      if (!key %in% private$.registry$get_field_names()) {
        cli_abort(
          paste0(
            "APIField '", key, "' not found in registry '",
      ...
      if (field$relation_type %in% c("one-to-many", "many-to-many")) {
        records_list <- APIRelatedRecords$new(
