...
APIRegistry <- R6::R6Class( # nolint object_name_linter
  "APIRegistry",
  cloneable = FALSE,
  public = list(
    #' @description
    ...
    initialize = function(instance, module, api, registry_name, registry_schema) {
      ...
      private$.fields <- purrr::map(
        ...
        purrr::set_names(names(registry_schema$fields_metadata))

      private$.record_class <- api_create_record_class(
        instance = instance,
        registry = self,
        api = api
      )
    ...
    from_df = function(dataframe, key = NULL, description = NULL, run = NULL) {
      ...
      api_create_record_from_python(py_record, private$.instance)
    },
    #' @description
    ...
    from_path = function(path, key = NULL, description = NULL, run = NULL) {
      if (isFALSE(private$.instance$is_default)) {
        cli::cli_abort(c(
          ...
          "i" = "Use {.code api_connect(slug = NULL)} to connect to the default instance"
        ))
      }
      ...
      if (is.null(private$.instance$get_py_lamin())) {
        cli::cli_abort(c(
          ...
          "i" = "Check the output of {.code api_connect()} for warnings"
        ))
      }
    ...
    #'
    #' @return A list of [APIField] objects.
    get_fields = function() {
      private$.fields
    },
    #' @description
    ...
    get_temporary_record_class = function() {
      if (is.null(private$.temporary_record_class)) {
        private$.temporary_record_class <- api_create_temporary_record_class(
          private$.record_class
        )
      }
    ...
    print = function(style = TRUE) {
      ...
      fields <- self$get_fields() |>
        ...
        purrr::discard(~ grepl("^_", .x$field_name)) |>
        # Remove link fields
        purrr::discard(~ grepl("^links_", .x$field_name))

      # Split fields into simple and relational
      simple_fields <- fields |>
        purrr::keep(~ is.null(.x$relation_type))
      relational_fields <- fields |>
        purrr::discard(~ is.null(.x$relation_type))
      ...
      simple_lines <-
        if (length(simple_fields) > 0) {
          c(
            ...
            purrr::map_chr(simple_fields, ~ paste0("    ", .x$field_name, ": ", .x$type))
          )
        } else {
          character(0)
        }
      ...
      relational_field_modules <- purrr::map_chr(relational_fields, "related_module_name")
      ...
      relational_lines <- purrr::map(related_modules, function(related_module_name) {
    ...
    #' @description
    #' Create a string representation of a `APIRegistry`
    #'
    #' @param style Logical, whether the output is styled using ANSI codes
    #'
    ...
    to_string = function(style = FALSE) {
      ...
      simple_strings <- make_key_value_strings(
        list(
          "SimpleFields" = paste0(
            "[",
            paste(purrr::map_chr(simple_fields, "field_name"), collapse = ", "),
            "]"
          )
        ),
        quote_strings = FALSE
      ...
      relational_strings <- purrr::map_chr(related_modules, function(related_module_name) {
        # get heading for module
        module_heading <-
          if (related_module_name == "core") {
            "RelationalFields"
        ...
        list(
          paste0("[", paste(purrr::map_chr(module_fields, "field_name"), collapse = ", "), "]")
        ) |>
          purrr::set_names(module_heading) |>
...
#' @noRd
api_create_record_from_python <- function(py_record, instance) {
  ...
  record_list <- purrr::map(fields, function(.field) {
    value <- tryCatch(
      py_record[[.field]],
      error = function(err) {
        NULL
    ...
    if (inherits(value, "lamindb.models.Record")) {
      value <- api_create_record_from_python(value, instance)
    }
    value
  }) |>
