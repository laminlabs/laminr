...
#' instance <- api_connect("laminlabs/cellxgene")
#' instance
#' }
api_connect <- function(slug = NULL) {
        ...
        error_msg <-
          if (is.null(slug)) {
            paste0(
              ...
              " - Provide a slug. For example: `api_connect(\"laminlabs/cellxgene\")`)\n",
  ...
  api_create_instance(instance_settings, is_default)
}
...
.connect_get_instance_settings <- function(owner, name, access_token) { # nolint object_length_linter
  ...
  content <- api_process_httr_response(request, "connect to instance")

  if (length(content) == 0) {
    cli_abort(paste0("Instance '", owner, "/", name, "' not found"))
  }

  APIInstanceSettings$new(content)
}
