...
api_generate_module_markdown <- function(db, module_name, allowed_related_modules = c("core", module_name)) {
  ...
  for (registry_name in registry_names) { # nolint cyclocomp_linter
    ...
    relational_fields <- fields |> purrr::keep(
      ~ !is.null(.x$related_field_name) &&
        !grepl("^_", .x$field_name) &&
        !.x$is_link_table &&
