...
This can also be useful if you want to use another R package which expects a particular Python module to be installed.
...
Using `connect(slug = NULL)` will connect to the current default LaminDB instance.
...
Note that this must be called before attempting to connect to default instance and cannot be changed once the default instance is connected.
